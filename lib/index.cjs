...
const _slotsToParts = (slots, options) => {
  ...
  if (allowedUnits) {
    ...
    let smallestUnitIndex = unitLookup[smallestUnit]
    if (smallestUnitIndex > largestUnitIndex) {
      smallestUnitIndex = largestUnitIndex
    }
...
class Duration {
  ...
  [toStringSymbol](options) {
    ...
    if (units === undefined) {
      largestUnit = largestUnit !== undefined ? toUnit(largestUnit) : 'day'
      smallestUnit = smallestUnit !== undefined ? toUnit(smallestUnit) : 'nanosecond'
    } else {
      units = toAllowedUnits(units)
    }
    ...
    let result = 'P'
    if (years) {
      result += `${years < 0 ? -years : years}Y`
    }
    if (quarters) {
      result += `${quarters < 0 ? -quarters : quarters}Q`
    }
    if (months) {
      result += `${months < 0 ? -months : months}M`
    }
    if (weeks) {
      result += `${weeks < 0 ? -weeks : weeks}W`
    }
    if (days) {
      result += `${days < 0 ? -days : days}D`
    }
    if (hours) {
      result += `T${hours < 0 ? -hours : hours}H`
    }
    if (minutes) {
      if (!hours) {
        result += 'T'
      }
      result += `${minutes < 0 ? -minutes : minutes}M`
    }
