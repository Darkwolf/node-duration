...
const _parseUnit = (string, targetUnit) => {
  ...
  return _convert(amount, unit, targetUnit)
}
const parseUnit = (string, unit) => {
  unit = unit !== undefined ? toUnit(unit) : UNIT
  string = String(string)
  return _parseUnit(string, unit)
}

const _parseUnits = (string, targetUnit) => {
...
const parseUnits = (string, unit) => {
  unit = unit !== undefined ? toUnit(unit) : UNIT
  string = String(string)
  return _parseUnits(string, unit)
}
...
const _parseTime = (string, unit, options) => {
  const match = StringPrototypeMatch(string, /^([+-])?(\d+):([0-5][0-9])(?::([0-5][0-9](?:\.\d{1,9})?))?$/)
  if (!match) {
    return NaN
  }
...
const toSlots = input => {
  ...
  if (InstancesIsDate(input)) {
    const milliseconds = DatePrototypeGetTime(input)
    if (NumberIsNaN(milliseconds)) {
      throw new RangeError('The value of the date must be a finite number')
    }
    return convertToSlots(milliseconds, 'millisecond')
  }
  return TypesIsPlainObject(input) ? partsToSlots(input) : durationToSlots(input)
}
...
const toDuration = (input, unit, options) => {
  ...
  if (InstancesIsDate(input)) {
    input = DatePrototypeGetTime(input)
    if (NumberIsNaN(input)) {
      throw new RangeError('The value of the date must be a finite number')
    }
    return _convert(input, 'millisecond', unit, options)
  }
